...
# Convert the data to a data frame and prepare for filtering
data <- as.data.frame(`GSPC`) %>%
  mutate(Date = index(`GSPC`)) %>%
  filter(Date >= as.Date(end_date) %m-% years(3) & Date <= as.Date(end_date))
...
metrics <- data.frame(Metric = c("RMSE", "MAE", "MAPE", "R_squared"),
...
# Create a function to perform rolling forecast for different windows
...
# Define the forecasting windows
window <- c("3-Day" = 3, "10-Day" = 10, "1-Month" = 20, "3-Month" = 60)

# Calculate forecasts for each horizon
results <- lapply(window, function(h) rolling_forecast(data, h))
...
row.names(combined_metrics) <- names(window)

...
names(stocks)[2] <- "Index"
stocks_clean <- na.omit(stocks) %>%
  mutate(Date = ymd(Date)) %>%
  filter(Date >= as.Date(Sys.Date()) - years(3))  # Filter to last 3 years
...
# Apply normalization function to cleaned data
...
features <- c("Index") #Feature to be used in training the model
...
# Visualization for both train and test data
train_data_plot <- train_data_index %>%
  select(Date, Index) %>%
  mutate(Data_Set = "Train")

test_data_plot <- test_data_index %>%
  select(Date, Index) %>%
  mutate(Data_Set = "Test")
...
history <- model %>% fit(train_data$x, train_data$y, epochs = 100, batch_size = 40,  validation_split = 0.1,  shuffle = FALSE)
summary(model)
...
ggplot(predictions_with_dates, aes(x = Date)) +
  ...
  labs(
    ...
    x = "Date", # Sets the y-axis label to "Date"
    y = "Index",  # Sets the y-axis label to "Index"
    colour = "Legend"  # Correct legend title
  ) +
  scale_colour_manual(
    values = c("Actual" = "blue", "Predicted" = "red"),  # Specify colors for the lines
...
forecast_errors <- rbind(
  ...
  calculate_forecast_errors(actual_values, predicted_values, forecast_window = 10),  
  calculate_forecast_errors(actual_values, predicted_values, forecast_window = 20), 
  calculate_forecast_errors(actual_values, predicted_values, forecast_window = 60)  
)
